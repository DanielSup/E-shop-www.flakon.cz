...
header('Content-Type: text/xml');
	...
	foreach ($query_products->rows as $product) {
		...
		JOIN " . DB_PREFIX . "category_description C ON A.category_id = C.category_id
		WHERE A.product_id =" .$product['product_id'] . "
		AND C.`language_id` =2
		");
		$shopitem .= "<CATEGORYTEXT>";
		$str="";
		foreach ($cat->rows as $ca) {
			$act = $ca["category_id"];
			$str.=$ca["name"];
			while($act != 0){
				$par = $db->query("SELECT * FROM " . DB_PREFIX ."category WHERE category_id=".$act);
				foreach($par->rows as $parC){
					$act=$parC["parent_id"];
				}
				if($act == 0){
					break;
				}
				$parr = $db->query("SELECT * FROM " . DB_PREFIX . "category_description WHERE category_id =" .$act." AND `language_id`=2");
				foreach($parr->rows as $parF){
					$str = $parF["name"]." | ".$str;
				}
			}
		}
		$shopitem .= $str."</CATEGORYTEXT>\n";
		$shopitem .= "<DELIVERY_DATE>0</DELIVERY_DATE>\n";
		$shopitem .="<MANUFACTURER>MARCEL ŠUP</MANUFACTURER>";
		$shopitem .= "<DELIVERY>\n";
		$shopitem .= "<DELIVERY_ID>CESKA_POSTA</DELIVERY_ID>\n";
		$shopitem .= "<DELIVERY_PRICE>100</DELIVERY_PRICE>\n";
		$shopitem .= " <DELIVERY_PRICE_COD>100</DELIVERY_PRICE_COD>\n";
		$shopitem .= "</DELIVERY>\n";
		$shopitem .= "<DELIVERY>\n";
		$shopitem .= "<DELIVERY_ID>GLS</DELIVERY_ID>\n";
		$shopitem .= "<DELIVERY_PRICE>100</DELIVERY_PRICE>\n";
		$shopitem .= " <DELIVERY_PRICE_COD>100</DELIVERY_PRICE_COD>\n";
		$shopitem .= "</DELIVERY>\n";
		if (!$product['image']==0) {
			$shopitem .= "<IMGURL>http://www.flakon.cz/image/".str_replace('%2F','/',rawurlencode($product['image']))."</IMGURL>\n";
			$shopitem .= "<IMGURL_ALTERNATIVE>http://www.flakon.cz/image/".str_replace('%2F','/',rawurlencode($product['image']))."</IMGURL_ALTERNATIVE>\n";
		}
		$shopitem .= "<PRICE_VAT>".round($product['price'], 0)."</PRICE_VAT>\n";
		$shopitem .= "<ITEMGROUP_ID>".$product['product_id']."</ITEMGROUP_ID>\n";
		$shopitem .= "</SHOPITEM>\n";
		$body .= $shopitem;
		echo $shopitem;
	}
