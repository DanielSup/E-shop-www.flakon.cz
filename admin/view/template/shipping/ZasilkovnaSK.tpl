...
$ZasilkovnaSK = new ControllerShippingZasilkovnaSK($registry);
?>
...
<div class="box">
  ...
  <div class="content">
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <?php
        for($i=0;$i<$ZasilkovnaSK->_servicesCnt;$i++) { ?>                  
          <tr>            
            <td style="align:left;">
                  
              <?php            
                $input_field_name = "ZasilkovnaSK_title_".$i;                       
              ?> 
            ...
            <td>                 
              <?php        
                $input_field_name = "ZasilkovnaSK_price_".$i;                       
              ?> 
              <input type="text" name="<?php echo $input_field_name;?>" value="<?php echo ${$input_field_name};?>" />
            </td>   
            ...
            <td>                 
              <?php        
                $input_field_name = "ZasilkovnaSK_freeover_".$i;                       
              ?> 
              <input type="text" name="<?php echo $input_field_name;?>" value="<?php echo ${$input_field_name};?>" />
            </td>

            <td>                 
              <?php $input_field_name = "ZasilkovnaSK_destination_".$i;  ?>
      ...
      <table class="form">
        <tr>
          ...
          <td><select name="ZasilkovnaSK_status">
              <?php if ($ZasilkovnaSK_status) { ?>
              <option value="1" selected="selected">ano</option>
              <option value="0">ne</option>
              <?php } else { ?>
              <option value="1">ano</option>
        ...
        <tr>
          <td><b>API klíč:</b></td>
          <td> <?php            
                $input_field_name = "ZasilkovnaSK_api_key";                      
              ?> 
              <input type="text" size="17" name="<?php echo $input_field_name;?>" value="<?php echo ${$input_field_name};?>" />
              <?php
              if(strlen(${$input_field_name})!=16){
                echo '<p style="color:red; font-weight:bold;">Zadajte správný api klíč! (Získáte na <a href="http://www.ZasilkovnaSK.cz/muj-ucet">ZasilkovnaSK.cz/muj-ucet</a></p>';
              }
              ?>
          </td>          
        </tr>        
          <td>Daň:</td>
          <td><select name="ZasilkovnaSK_tax_class_id">
              <option value="0"><?php echo $text_none; ?></option>
              <?php foreach ($tax_classes as $tax_class) { ?>
              <?php if ($tax_class['tax_class_id'] == $ZasilkovnaSK_tax_class_id) { ?>
        ...
        <tr>
          ...
          <td>
              <?php $input_field_name = "ZasilkovnaSK_weight_max"; ?>
              <input type="text" size="17" name="<?php echo $input_field_name;?>" value="<?php echo isset(${$input_field_name}) ? ${$input_field_name} : '';?>" />
          </td>
        </tr>
        <tr>
          <td>Geo zone</td>
          <td><select name="ZasilkovnaSK_geo_zone_id">
              <option value="0"><?php echo $text_all_zones; ?></option>
              <?php foreach ($geo_zones as $geo_zone) { ?>
              <?php if ($geo_zone['geo_zone_id'] == $ZasilkovnaSK_geo_zone_id) { ?>
        ...
        <tr>
          <td>Priorita:</td>
          <td><input type="text" name="ZasilkovnaSK_sort_order" value="<?php echo $ZasilkovnaSK_sort_order; ?>" size="1" /></td>
        </tr>
      </table>
    </form>
  </div>
